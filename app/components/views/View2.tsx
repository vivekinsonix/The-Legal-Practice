            ...
            {info_cards?.length > 0 && (
              <div role="list" className="my-6 grid gap-6 sm:grid-cols-2 lg:grid-cols-3 ">
                {info_cards?.map((f: any, index: number) => (
                  <div role="listitem" key={f?.title + index} className="rounded-3xl bg-white dark:bg-secondary-dark p-6 shadow-lg hover:scale-105 transition-all">
                    {f?.icon && (
                      <span aria-hidden="true" className="inline-flex items-center justify-center rounded-3xl bg-slate-700 p-2 text-white">
